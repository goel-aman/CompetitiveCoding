...
//  order are unique and were sorted in some custom order 
// previously.
// Permute the characters of s so that they match the order
//  that order was sorted. More specifically, if a character
//  x occurs before a character y in order, then x should occur
...
class compare{
    ...
    public:
    compare(unordered_map<char,int> input){
        um = input;
    }

    bool operator()(char& a,char& b){
        if(um.count(a) > 0 && um.count(b) > 0){
            return (um[a] < um[b]);          
        }
