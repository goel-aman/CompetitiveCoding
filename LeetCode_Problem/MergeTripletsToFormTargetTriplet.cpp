...
public:
    bool mergeTriplets(vector<vector<int>>& triplets, vector<int>& target) {
        ...
        int firstTarget = target[0],secondTarget = target[1],thirdTarget = target[2];
        bool first = false, second = false, third = false;
        for(int i=0;i<tripletsLength;i++){
        
            if((triplets[i][0] > firstTarget) || (triplets[i][1] > secondTarget) || (triplets[i][2] > thirdTarget)){
                continue;
            }

            if(triplets[i][0] == firstTarget){
                first = true;
            }

            if(triplets[i][1] == secondTarget){
                second = true;
            }

            if(triplets[i][2] == thirdTarget){
                third = true;
            }
        }
