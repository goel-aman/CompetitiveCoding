...
// are leaving building 0, one is leaving building 1, and one is leaving building 2, there
//  should be two employees moving to building 0, one employee moving to building 1, and
...
class Solution {
    ...
    int ans;
public:
    void answer(vector<vector<int>>& requests,int index,int num,unordered_map<int,int> &um){
        ...
        // considering the current request.
        um[requests[index][0]]--;
        um[requests[index][1]]++;
        answer(requests,index + 1,num + 1,um);
        um[requests[index][0]]++;
        um[requests[index][1]]--;

        // skipping the current request

        answer(requests,index + 1,num,um);
    }

    int maximumRequests(int n, vector<vector<int>>& requests) {
        requestLength = requests.size();
        ans = 0;
        answer(requests,0,0,um);
        return ans;
    }
};