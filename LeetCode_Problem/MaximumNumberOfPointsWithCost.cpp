...
// Starting with 0 points, you want to maximize the number of
// points you can get from the matrix.

// To gain points, you must pick one cell in each row. Picking 
// the cell at coordinates (r, c) will add points[r][c] to your
//  score.
...
//  at coordinates (r, c1) and (r + 1, c2) will subtract 
...
public:
    long long maxPoints(vector<vector<int>>& points) {
        ...
        vector<long long > prevRow(cols),currentRow(cols);
        for(auto var : points){
            for(long long i=0,maxNum = 0;i<cols;i++){
                maxNum = max(maxNum - 1,prevRow[i]);
                currentRow[i] = maxNum;
            }
            
            for(long long i=cols - 1,maxNum = 0;i >= 0;i--){
                maxNum = max(maxNum - 1,prevRow[i]);
                currentRow[i] = max(maxNum,currentRow[i]) + var[i];
            }
