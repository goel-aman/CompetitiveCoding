...
// of the array by 1. The player adds the chosen number to their score.
...
class Solution {
    ...
    int numsLength;
public:
    // function will return the score of the player whose turn is there.
    int PredictTheWinner(vector<int>& nums,int start,int end,int sum){
        if(start > end){
            return 0;
        }
        ...
        int value1 = (sum - PredictTheWinner(nums,start + 1,end,sum - nums[start]));
        int value2 = (sum - PredictTheWinner(nums,start,end - 1,sum - nums[end]));
    ...
    bool PredictTheWinner(vector<int>& nums) {
        ...
        int value = PredictTheWinner(nums,0,numsLength - 1,sum);
        if(value >= (sum - value)){
            return true;
        }
        return false;
