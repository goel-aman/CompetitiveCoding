...
public:
    ...
    int getAnswer(vector<vector<int>> &grid,int row ,int col){
        if(grid[row][col] == 0){
            return 0;
        }
        ...
        if(row + 1 < rows & row + 1 >= 0){
            value = max(value,getAnswer(grid,row + 1,col));
        }   
        if(row - 1 < rows && row - 1 >= 0){
            value = max(value,getAnswer(grid,row - 1,col));
        }
        ...
        if(col + 1 < cols && col >= 0){
            value = max(value,getAnswer(grid,row,col + 1));
        }
    ...
    int getMaximumGold(vector<vector<int>>& grid) {
        ...
        for(int i=0;i<rows;i++){
            for(int j=0;j<cols;j++){
                if(grid[i][j] != 0){
                    ans = max(ans,getAnswer(grid,i,j));
                }
            }
        }
        return ans;
