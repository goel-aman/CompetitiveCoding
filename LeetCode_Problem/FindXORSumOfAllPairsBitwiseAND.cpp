...
//  If the list only contains one element, then its XOR sum will
//  be equal to this element.
// For example, the XOR sum of [1,2,3,4] is equal to 1 XOR 2 XOR
//  3 XOR 4 = 4, and the XOR sum of [3] is equal to 3.
// You are given two 0-indexed arrays arr1 and arr2 that consist 
...
public:
    int getXORSum(vector<int>& arr1, vector<int>& arr2) {
        ...
        for(auto var1 : first){
            if(var1.second%2 == 1){
                for(auto var2 : second){
                    if(var2.second%2 == 1){
                        ans = (ans^(var1.first&var2.first));
                    }
                }
            }
        }
