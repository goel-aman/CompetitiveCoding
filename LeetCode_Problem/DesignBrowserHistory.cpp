...
// you can visit another url, get back in the history number of steps
...
//  x steps. Return the current url after moving back in history at 
// most steps.
...
public:
    ...
    void visit(string url) {
        totalPages = currentIndex + 1 + 1;
        if(array.size() < totalPages){
            array.push_back(url);
        }else{
            array[currentIndex + 1] = url;
        }
        currentIndex++;
        return;
    }
    ...
    string forward(int steps) {
        currentIndex = min(currentIndex + steps,totalPages - 1);
        return array[currentIndex];
    }
};
