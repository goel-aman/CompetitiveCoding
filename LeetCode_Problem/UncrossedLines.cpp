...
// You are given two integer arrays nums1 and nums2. We write 
// the integers of nums1 and nums2 (in the order they are 
// given) on two separate horizontal lines.

// We may draw connecting lines: a straight line connecting 
...
// endpoints (i.e., each number can only belong to one 
// connecting line).
...
// nums1[1] = 4 to nums2[2] = 4 will intersect the line from
...
public:
    int maxUncrossedLines(vector<int>& nums1, vector<int>& nums2) {
        ...
        int ans = 0;
        vector<vector<int>> dp(nums1Length + 1,vector<int> (nums2Length + 1,0));
