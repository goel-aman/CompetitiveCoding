...
public:
    int divide(int dividend, int divisor) {
        ...
        long dividendd = labs(dividend);
        long divisorr = labs(divisor);
        long answer = 0;
        while(dividendd - divisorr >= 0){
            int count = 0;
            while( dividendd -  (divisorr << 1 << count) >= 0){
                count++;
            }
            // cout<<" value of count is : "<<count<<endl;
            // cout<<" 1 << count is : "<<(1<<count)<<endl;
            answer += (1 << count);
            dividendd -= (divisorr << count);
            // cout<<"dividend is : "<<dividend<<" and divisor is : "<<divisor<<endl;
        }
        if(isPositive){
            return answer;
        }
