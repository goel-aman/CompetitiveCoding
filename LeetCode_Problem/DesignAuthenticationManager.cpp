...
// There is an authentication system that works with 
// authentication tokens. For each session, the user 
// will receive a new authentication token that will 
// expire timeToLive seconds after the currentTime. If
//  the token is renewed, the expiry time will be extended
//  to expire timeToLive seconds after the (potentially
...
// generate(string tokenId, int currentTime) generates a
//  new token with the given tokenId at the given currentTime
//  in seconds.
// renew(string tokenId, int currentTime) renews the 
// unexpired token with the given tokenId at the given currentTime
...
// Note that if a token expires at time t, and another action 
// happens on time t (renew or countUnexpiredTokens), the 
...
//  "bbb" expires at time 15, and the token with tokenId "aaa" expired at 
...
// The values of currentTime across all the function calls 
...
public:
    ...
    void generate(string tokenId, int currentTime) {
        um[tokenId] = currentTime + toLive;
    }
    
    void renew(string tokenId, int currentTime) {
        if(um.count(tokenId)){
            if(currentTime < um[tokenId]){
                um[tokenId] = currentTime + toLive;
            }
        }
        return; 
    ...
    int countUnexpiredTokens(int currentTime) {
        int ans = 0;
        for(auto var : um){
            // cout<<"var.first : "<<var.first<<"  var.second is :  "<<var.second<<endl;
            if(currentTime < var.second){
                ans += 1;
            }
        }
