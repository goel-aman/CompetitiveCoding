...
// follows: choose any node, and swap the left and right 
// child subtrees.

// A binary tree X is flip equivalent to a binary tree Y 
// if and only if we can make X equal to Y after some number
//  of flip operations.
...
public:
    bool flipEquiv(TreeNode* root1, TreeNode* root2) {
        ...
        bool value = (flipEquiv(root1->left,root2->left) && flipEquiv(root1->right,root2->right)) || (flipEquiv(root1->left,root2->right) && flipEquiv(root1->right,root2->left));
        if(value){
            return true;
        }
        return false;
