...
// You are given two non-increasing 0-indexed integer
//  arrays nums1​​​​​​ and nums2​​​​​​.

// A pair of indices (i, j), where 0 <= i < nums1.length
//  and 0 <= j < nums2.length, is valid if both i <= j and
...
// An array arr is non-increasing if arr[i-1] >= arr[i] 
...
public:
    int maxDistance(vector<int>& nums1, vector<int>& nums2) {
        ...
        for(int i=0;i<nums1Length;i++){
            ...
            int updatedIndex = nums2Length - 1 - index;
            if(i <= updatedIndex){
                ans = max(ans,updatedIndex - i);
            }
        }
        return ans;
    }
