...
// SeatManager(int n) Initializes a SeatManager object that will
//  manage n seats numbered from 1 to n. All seats are initially
//  available.
...
// seatManager.reserve();    // All seats are available, so return
//  the lowest numbered seat, which is 1.
// seatManager.reserve();    // The available seats are [2,3,4,5], 
// so return the lowest of them, which is 2.
...
public:
    ...
    int reserve() {
        int value = *sFilled.begin();
        sFilled.erase(value);
        return value;
    }
    
    void unreserve(int seatNumber) {
        sFilled.insert(seatNumber);
        return; 
    }
};
