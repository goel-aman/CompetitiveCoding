...
public:
    ...
    bool compute(vector<int>& gas,vector<int>& cost,int index,int currentFuel,vector<int>& visited){
        if(visited[index] == 1){
            return true;
        }

        visited[index] = 1;
        currentFuel += gas[index];
        if(currentFuel < cost[index]){
            visited[index] = -1;
            return false;
        }

        int nextIndex = (index + 1) % gasLength;
        bool ans = compute(gas,cost,nextIndex,currentFuel - cost[index],visited);
        if(ans == true){
            visited[index] = -1;
            return true;   
        }
    ...
    int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
        gasLength = gas.size();
        vector<int> visited(gasLength,-1);
        for(int i=0;i<gasLength;i++){
            bool ans =  compute(gas,cost,i,0,visited);
            if(ans){
                return i;
            }         
        }
