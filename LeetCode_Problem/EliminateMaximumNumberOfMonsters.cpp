...
// You are playing a video game where you are defending 
// your city from a group of n monsters. You are given a
//  0-indexed integer array dist of size n, where dist[i]
//  is the initial distance in kilometers of the ith monster
//  from the city.
// The monsters walk toward the city at a constant speed. The
// speed of each monster is given to you in an integer array 
// speed of size n, where speed[i] is the speed of the ith 
...
// You lose when any monster reaches your city. If a monster 
// reaches the city at the exact moment the weapon is fully 
// charged, it counts as a loss, and the game ends before you
//  can use your weapon.
...
public:
    int eliminateMaximum(vector<int>& dist, vector<int>& speed) {
        int distanceLength = dist.size();
        priority_queue<double,vector<double>,greater<double>> pq;
        for(int i=0;i<distanceLength;i++){
            pq.push(dist[i]/(double)speed[i]);
        }
        int ans = 0;
        int minutePassed = 0;
        while(!pq.empty()){
