...
// of groups. Each person is labeled with a unique ID 
// from 0 to n - 1.

// You are given an integer array groupSizes, where groupSizes[i] 
// is the size of the group that person i is in. For example, if 
...
// them. It is guaranteed that there will be at least one valid solution
//  for the given input.
...
public:
    vector<vector<int>> groupThePeople(vector<int>& groupSizes) {
        ...
        for(auto &var : um){
            int length = var.first;
            int i = 0;
            vector<int> temp;
            for(;i<var.second.size();i++){
                if(i%length == 0){
                    if(temp.size() != 0){
                        ans.push_back(temp);
                    }
