...
// edges[i] = [ai, bi] means that exists an edge connecting the vertices
//  ai and bi. Additionally, there is a boolean array hasApple, where
//  hasApple[i] = true means that vertex i has an apple; otherwise, 
...
public:
    ...
    int traverse(int root){
        int ans = 0;
        for(auto element : um[root]){
            ans += traverse(element);
        }
    ...
    int minTime(int n, vector<vector<int>>& edges, vector<bool>& hasApple) {
        for(int i=0;i<edges.size();i++){
            um[edges[i][0]].push_back(edges[i][0]);
        }
        apple = hasApple;
        int val = traverse(0);
        if(apple[0]){
