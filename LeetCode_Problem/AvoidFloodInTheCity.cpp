...
// but when it rains over the nth lake, the nth lake becomes full of water. If it rains
//  over a lake which is full of water, there will be a flood. Your goal is to avoid the
//  flood in any lake.
...
public:
    vector<int> avoidFlood(vector<int>& rains) {
        int rainsLength = rains.size();
        unordered_map<int,int> visited;
        vector<int> daysZero;
        vector<int> ans;

        for(int i=0;i<rainsLength;i++){
            if(rains[i] == 0){
                daysZero.push_back(i);
                ans.push_back(1);
            }
            else{
                if(visited.find(rains[i]) != visited.end()){
                    auto it = lower_bound(daysZero.begin(),daysZero.end(),visited[rains[i]]);
                    if(it != daysZero.end()){
                        ans[*it] = rains[i];
                        daysZero.erase(it);
                        visited[rains[i]] = i;
