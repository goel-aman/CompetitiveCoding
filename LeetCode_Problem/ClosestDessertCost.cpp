...
public:
    ...
    void closestCost(vector<int>& toppingCosts,int target,int index2,int value){
        if(index2 == toppingLength){
        ...
        for(int j = 0;j<=2;j++){
            closestCost(toppingCosts,target,index2 + 1,value + (toppingCosts[index2] * j));
        }
        return ;
    }

    int closestCost(vector<int>& baseCosts, vector<int>& toppingCosts, int target) {
        baseLength = baseCosts.size();
        toppingLength = toppingCosts.size();
        for(int i=0;i<baseLength;i++){
            closestCost(toppingCosts,target,0,baseCosts[i]);
        }
        return ans;
    }
};