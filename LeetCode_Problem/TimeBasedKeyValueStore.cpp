...
// timeMap.set("foo", "bar2", 4); // store the key "foo" and value "ba2r" 
...
public:
    ...
    string get(string key, int timestamp) {
        ...
        auto iterator = um[key].lower_bound(timestamp);
        if(iterator != um[key].end()){
            if((*iterator).first == timestamp){
                return (*iterator).second;
            }
            
            if(iterator == um[key].begin()){
                return "";
            }
            iterator--;
            return (*iterator).second;
