...
// Under the grammar given below, strings can represent 
// a set of lowercase words. Let's use R(expr) to denote 
...
//  concatenations between two words where the first word comes from 
// the first expression and the second word comes from the second 
// expression.
...
//  R({e1, e2, ...}) = R(e1) ∪ R(e2) ∪ ...
// For expressions e1 and e2, we have R(e1 + e2) = {a + b for 
// (a, b) in R(e1) × R(e2)}, where + denotes concatenation, and × 
...
// Output: ["a","ab","ac","z"]
// Explanation: Each distinct word is written only 
// once in the final answer.
 
// Constraints:
// 1 <= expression.length <= 60
// expression[i] consists of '{', '}', ','or 
// lowercase English letters.
// The given expression represents a set of 
// words based on the grammar given in the description.

class Solution {
public:
    vector<string> helper(string expression){
