...
//  of their own, and so on, to produce a multilevel data structure, as shown 
...
public:
    Node* flatten(Node* head) {
        ...
        Node* ans = head;
        Node* tempHead = head;
        while(tempHead->next != NULL && tempHead->child == NULL){
            tempHead = tempHead->next;
        }

        if(tempHead->child != NULL){
            Node* newNode = flatten(tempHead->child);
            Node* saveNext = tempHead->next;
