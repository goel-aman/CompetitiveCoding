...
//  of some people in a queue (not necessarily in order). Each
//  people[i] = [hi, ki] represents the ith person of height hi with
//  exactly ki other people in front who have a height greater than
//  or equal to hi.
...
// Explanation:
// Person 0 has height 5 with no other people taller
//  or the same height in front.
// Person 1 has height 7 with no other people taller
//  or the same height in front.
// Person 2 has height 5 with two persons taller or the 
// same height in front, which is person 0 and 1.
// Person 3 has height 6 with one person taller or the 
// same height in front, which is person 1.
// Person 4 has height 4 with four people taller or 
...
public:
    vector<vector<int>> reconstructQueue(vector<vector<int>>& people) {
        ...
        vector<vector<int>> ans(peopleLength,vector<int> (2,-1));
