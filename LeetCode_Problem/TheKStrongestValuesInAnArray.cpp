...
// A value arr[i] is said to be stronger than a value 
// arr[j] if |arr[i] - m| > |arr[j] - m| where m is the
//  median of the array.
// If |arr[i] - m| == |arr[j] - m|, then arr[i] is said
//  to be stronger than arr[j] if arr[i] > arr[j].
...
// obtained by sorting the array arr = [-3, 2, 6, 7, 11] and 
// the median is arr[m] where m = ((5 - 1) / 2) = 2. The 
// median is 6.
// For arr = [-7, 22, 17, 3], n = 4 and the median is obtained 
// by sorting the array arr = [-7, 3, 17, 22] and the median is
...
// the strongest are [5,1,4,2,3]. The strongest 2 elements are
//  [5, 1]. [1, 5] is also accepted answer.
// Please note that although |5 - 3| == |1 - 3| but 5 is stronger
//  than 1 because 5 > 1.
...
bool compare(int &a,int &b){
    ...
    if(abs(a - value) == abs(b - value) && a > b){
        return true;
    }
