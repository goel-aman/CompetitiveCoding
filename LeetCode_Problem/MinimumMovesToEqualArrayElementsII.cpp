...
//  of the array by 1.
// Test cases are designed so that the answer will fit in
// a 32-bit integer.
...
// Output: 2
// Explanation:
// Only two moves are needed (remember each move increments
...
public:
    int minMoves2(vector<int>& nums) {
        ...
        int centerValue;
        if(numsLength%2 == 0){
            centerValue = (nums[mid] + nums[mid - 1])/2; 
        }
        else{
            centerValue = nums[mid];
        }

        int ans = 0;
        for(int i=0;i<numsLength;i++){
            ans += abs(nums[i] - centerValue);
        }
        return ans;
    }
};