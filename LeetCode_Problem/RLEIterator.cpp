...
// number of times that the non-negative integer value encoding[i + 1] 
...
// 0element exhausted in this way. If there is no element left to 
...
class RLEIterator {
    // first show counter,number
    vector<pair<long long int,long long int>> decoded;
    int length;
    long long int lastConsumedIndex;
    long long int prevIndex = 0;
    long long int lastIndex;
public:
    RLEIterator(vector<int>& encoding) {
        long long int encodingLength = encoding.size();
        long long int value = 0;
        
        for(int i=0;i<(encodingLength/2);i++){
            long long int times = encoding[2 * i];
            long long int number = encoding[(2 * i) + 1];
            if(times != 0){
                value += times;
                decoded.push_back({value,number});
            }
        }
        lastConsumedIndex = 0;
        lastIndex = decoded.size() - 1;
    }
    
    int next(int n) {
        lastConsumedIndex += n;
        int start = prevIndex , end = lastIndex;
        int ans = -1;
        while(start <= end){
            int mid = (start + end)/2;
            if(decoded[mid].first >= lastConsumedIndex){
                ans = mid;
                end = mid - 1;
                continue;
            }
