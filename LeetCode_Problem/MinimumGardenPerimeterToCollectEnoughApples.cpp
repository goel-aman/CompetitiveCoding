...
// In a garden represented as an infinite 2D grid, there is an apple
//  tree planted at every integer coordinate. The apple tree planted 
// at an integer coordinate (i, j) has |i| + |j| apples growing on it.
...
public:
    long long minimumPerimeter(long long neededApples) {
        ...
        for(;neededApples > 0;i++){
            neededApples -= (12 * i);
            neededApples -= (((2*i - 1)*(2*i)/2) - ((i)*(i + 1)/2)) * 8; 
        }
        return (i - 1) * 8;
    }
};