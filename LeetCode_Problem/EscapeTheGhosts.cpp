...
// point target = [xtarget, ytarget] that you are trying to get to.
//  There are several ghosts on the map with their starting positions 
// given as a 2D array ghosts, where ghosts[i] = [xi, yi] represents
//  the starting position of the ith ghost. All inputs are integral 
// coordinates.
...
// Output: false
// Explanation: You need to reach the destination (2, 0), but the ghost
//  at (1, 0) lies between you and the destination.
...
public:
    bool escapeGhosts(vector<vector<int>>& ghosts, vector<int>& target) {
        int DistanceTarget = abs(target[0] - 0) + abs(target[1]);
        int ghostLength = ghosts.size();
        vector<int> distanceGhosts;
        for(int i=0;i<ghostLength;i++){
            int distance = abs(target[0] - ghosts[i][0]) + abs(target[1] - ghosts[i][1]);
