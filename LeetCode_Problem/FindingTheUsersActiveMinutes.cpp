...
// integer array logs where each logs[i] = [IDi, timei] 
// indicates that the user with IDi performed an action at
//  the minute timei.
...
// The user active minutes (UAM) for a given user is 
// defined as the number of unique minutes in which the user
// performed an action on LeetCode. A minute can only be 
// counted once, even if multiple actions occur during it.
...
// Output: [0,2,0,0,0]
// Explanation:
// The user with ID=0 performed actions at minutes 5, 2, and 5 
// again. Hence, they have a UAM of 2 (minute 5 is only counted once).
...
// Output: [1,1,0,0]
// Explanation:
// The user with ID=1 performed a single action at minute 1.
//  Hence, they have a UAM of 1.
// The user with ID=2 performed actions at minutes 2 and 3. 
// Hence, they have a UAM of 2.
// There is one user with a UAM of 1 and one with a UAM of 2.
// Hence, answer[1] = 1, answer[2] = 1, and the remaining
//  values are 0.
...
public:
    vector<int> findingUsersActiveMinutes(vector<vector<int>>& logs, int k) {
        ...
        vector<int> ans(k,0);
        for(auto &var : uam){
            int activeMinutes = var.second.size();
            if(activeMinutes <= k && activeMinutes >= 1){
                ans[activeMinutes - 1]++;
            }
        }
        return ans;
