...
//  each person has a different amount of money and a different
//  level of quietness.

// You are given an array richer where richer[i] = [ai, bi] indicates
//  that ai has more money than bi and an integer array quiet where 
// quiet[i] is the quietness of the ith person. All the given data in
//  richer are logically correct (i.e., the data will not lead you to
//  a situation where x is richer than y and y is richer than x at the
//  same time).
...
// it is not clear if they have more money than person 0.
// answer[7] = 7.
...
class Solution {
    vector<int> quiett;
public:
    void graphSolve(int source,unordered_map<int,vector<int>>& um,unordered_map<int,int>& solution){
        int least_answer = source;

        for(auto var : um[source]){
            if(!solution.count(var)){
                graphSolve(var,um,solution);
            }

            if(quiett[solution[var]] < quiett[least_answer]){
                least_answer = solution[var];
            }
        }        
        solution[source] = least_answer;
        return ; 
    }
    
    vector<int> loudAndRich(vector<vector<int>>& richer, vector<int>& quiet) {
        quiett = quiet;
        int richerLength = richer.size();
        int quietLength = quiet.size();
        
        unordered_map<int,vector<int>> um;

        for(int i=0;i<richerLength;i++){
            int moreMoneyPerson = richer[i][0];
            int lessMoneyPerson = richer[i][1];
            um[lessMoneyPerson].push_back(moreMoneyPerson);
        }
        unordered_map<int,int> solution;
       
        for(int i=0;i<quietLength;i++){
            if(!solution.count(i)){
                graphSolve(i,um,solution);
            }
        }
        vector<int> ans;
        for(int i=0;i<quietLength;i++){
            ans.push_back(solution[i]);
        }
        return ans;
    }
};