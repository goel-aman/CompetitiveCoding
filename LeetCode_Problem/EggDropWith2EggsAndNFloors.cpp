...
// You are given two identical eggs and you have access
//  to a building with n floors labeled from 1 to n.
// You know that there exists a floor f where 0 <= f <= n
//  such that any egg dropped at a floor higher than f will 
...
//  floor x (where 1 <= x <= n). If the egg breaks, you can no longer
//  use it. However, if the egg does not break, you may reuse 
// it in future moves.
...
public:
    int solve(int n,int eggsLeft){
        ...
        if(dp[n][eggsLeft] != -1){
            return dp[n][eggsLeft];
        }

        int ans = INT_MAX;
        for(int i=1;i<n;i++){
            ans = min(ans,1 + max(solve(i - 1,eggsLeft - 1) , solve(n - i,eggsLeft)));
        }
        return dp[n][eggsLeft]= ans;
    }    
