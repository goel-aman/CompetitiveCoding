...
public:
    ...
    double champagneTower(int poured, int query_row, int query_glass) {
        vector<double> vec;
        vec.push_back((double) poured);
        while(query_row--){
            vector<double> temp;
            temp.push_back(max((double) 0,(vec[0] - 1)/2));
            for(int i=1;i<vec.size();i++){
                temp.push_back( max((double)0,((vec[i - 1] - 1) / 2)) + max((double) 0,((vec[i] - 1) /2)) );
            }
            temp.push_back(max((double) 0,(vec[0] - 1)/2));
            vec = temp;
        }

        double ans = min((double) 1,vec[query_glass]);
        return ans;
    }
};
