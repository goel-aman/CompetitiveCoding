...
// the arrays watchedVideos and friends, where watchedVideos[i] and friends[i] 
...
public:
    vector<string> watchedVideosByFriends(vector<vector<string>>& watchedVideos, vector<vector<int>>& friends, int id, int level) {
        ...
        que.push(id);
        visited[id] = true;
        while(level > 0 && !que.empty()){
            int siz = que.size();
            while(siz--){
                int front = que.front();
                que.pop();

                for(auto element : friends[front]){
        ...
        while(!que.empty()){
            ...
            for(auto val : watchedVideos[front]){
                um[val]++;
            }                    
        }
        vector<elem> vec;
