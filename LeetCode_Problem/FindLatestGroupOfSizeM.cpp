...
//  find the latest step at which there exists a group of ones of length m. A group of 
// ones is a contiguous substring of 1s such that it cannot be extended in either direction.
...
public:
    int findLatestStep(vector<int>& arr, int m) {
        ...
        vector<int> vec(arrLength + 2,0);
        for(int i=0;i<arrLength;i++){
            int a = arr[i];
            int left = vec[a - 1];
            int right = vec[a + 1];
            vec[a - left] = vec[a + right] = left + right + 1;
            if(left == m || right == m){
                answer = i;
            }
        }
