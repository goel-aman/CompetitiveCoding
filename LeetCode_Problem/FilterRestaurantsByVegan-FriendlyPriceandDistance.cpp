...
// 1333. Filter Restaurants by Vegan-Friendly, 
...
//  we have restaurant 3, restaurant 1 and restaurant 5 (ordered by rating from 
// highest to lowest). 
...
public:
    vector<int> filterRestaurants(vector<vector<int>>& restaurants, int veganFriendly, int maxPrice, int maxDistance) {
        vector<int> ans;
        vector<elem> restaurant;
        if(veganFriendly == 1){
            for(auto var : restaurants){
                if(var[2] == 1 && var[3] <= maxPrice && var[4] <= maxDistance){
                    restaurant.push_back({var[0],var[1],var[2],var[3],var[4]});
                }
            }
        }else{
            for(auto var : restaurants){
                if(var[3] <= maxPrice && var[4] <= maxDistance){
                    restaurant.push_back({var[0],var[1],var[2],var[3],var[4]});
                }
            }
        }

        sort(restaurant.begin(),restaurant.end(),comp);
        for(auto var : restaurant){
            ans.push_back(var.id);
        }
        return ans;
    }
};