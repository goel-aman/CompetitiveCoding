...
//  tree, and the number of nodes n in the tree.  n is odd, and each node has a distinct value from 1 to n.
...
public:
    ...
    int numberOfNodes(TreeNode* root){
        ...
        int left = numberOfNodes(root->left), right = numberOfNodes(root->right);
        if(root->val == val){
            l = left;
            r = right;
            return 0;
    ...
    bool btreeGameWinningMove(TreeNode* root, int n, int x) {
        ...
        numberOfNodes(root);
        int upper = n - 1 - l - r;
        int maximumValue = max(l,max(r,upper));
        if(maximumValue > (n / 2)){
            return true;
        }
        return false;
