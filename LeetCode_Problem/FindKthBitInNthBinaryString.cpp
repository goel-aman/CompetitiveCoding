...
// and invert(x) inverts all the bits in x (0 
// changes to 1 and 1 changes to 0).

// For example, the first 4 strings in the
//  above sequence are:
...
// S3 = "0111001"
// S4 = "011100110110001"
// Return the kth bit in Sn. It is guaranteed that
//  k is valid for the given n.
...
public:
    ...
    char findKthBit(int n, int k) {
        string initial = "0";
        for(int i=1;i<n;i++){
            string inverted = invert(initial);
            reverse(inverted.begin(),inverted.end());
            string next = initial + "1" + inverted;
            initial = next;
        }
