...
//  the line, you should find the minimum number of moves to make all the washing machines have the same 
...
public:
    int findMinMoves(vector<int>& machines) {
        ...
        for(int i=0;i<length;i++){
            needCount += machines[i] - avg;
            final_ans = max(final_ans,max(abs(needCount),machines[i] - avg));
        }
        return final_ans;
    }
