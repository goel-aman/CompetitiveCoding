...
// represented as an array stations where stations[i] = [positioni, fueli] 
// pindicates that the ith gas station is positioni miles east of the 
...
public:
    int minRefuelStops(int target, int startFuel, vector<vector<int>>& stations) {
        int stationsLength = stations.size();
        int position = startFuel;
        int count = 0;
        int index = 0;
        priority_queue<int> pq;
        
        for(int i=index;i<stationsLength;i++){
        ...
        while(true){
            ...
            for(int i=index + 1;i<stationsLength;i++){
               if(stations[i][0] <= position){
                    pq.push(stations[i][1]);
                    index = i;
                }
