...
// You are given a floating-point number hour, representing the 
// amount of time you have to reach the office. To commute to the
// office, you must take n trains in sequential order. You are also
//  given an integer array dist of length n, where dist[i] describes
...
public:
    int minSpeedOnTime(vector<int>& dist, double hour) {
        int distLength = dist.size();
        if(hour <= distLength - 1){
            return -1;
        }
        int ans = INT_MAX;
        int start = 0 ,end = 1e7;
        while(start <= end){
            ...
            for(int i=0;i<distLength;i++){
                if(i != distLength - 1){
                    timeNeeded += ceil((double) dist[i]/ (double) mid);
                }
                else{
                    timeNeeded += ((double) dist[i]/ (double) mid);
                }
            }
            if(timeNeeded <= hour){
                ans = min(ans,mid);
