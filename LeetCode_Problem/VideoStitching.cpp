...
// You are given a series of video clips from a sporting
//  event that lasted time seconds. These video clips can
// be overlapping with each other and have varying lengths.

// Each video clip is described by an array clips where 
// clips[i] = [starti, endi] indicates that the ith clip 
...
// cut the clips into segments that cover the entire sporting
...
// Now we have segments [0,2] + [2,8] + [8,10] which cover the
//  sporting event [0, 10].
...
public:
    int videoStitching(vector<vector<int>>& clips, int T) {
        ...
        int clipsLength = clips.size(), currentTime = 0 , totalTaken = 0;
        int maxYouCan = -1;
        int i = 0;
        int gotAnswer = 0;
        for(;i<clipsLength;i++){
            if(clips[i][0] <= currentTime){
                maxYouCan = max(maxYouCan,clips[i][1]);
            }else{
                if(maxYouCan == -1){
                    break;
                }
